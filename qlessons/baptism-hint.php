...
// edit to clear errors
	...
	$stck2 = getQls()->SQL->query("SELECT * FROM `purchase_records` WHERE `user_id`= $id AND `title` LIKE 'Expedited Pro%'");
	$sttCk2 = getQls()->SQL->num_rows($stck2); // determine if Expedited is already bought
		if ($sttCk2 ==0) {
			  //buy it now..
