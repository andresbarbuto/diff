...
<title>
	Quinceanera 3  Quiz Retake Assigned
</title>
<?php
include '../start.php'; ?>
<a name="top"></a>
...
//echo $id . "id<br/>";
$lesson_id = 747; // set lesson_id for no accidents
//echo $lesson_id . " lesson<br/>";
?>

<h1 class="static-title">Lesson 747 Quiz Retake Assigned</h1>
		<p>
		<?
...
$lupdate = getQls()->SQL->query("UPDATE quiz_results SET may_retake=1 WHERE user_id=$id AND lesson_id='$lesson_id' ORDER BY id DESC LIMIT 1;") or die(getQls()->SQL->error());
		...
		?>
<input type="button" value="Return to Lesson" class="btn btn-success mb-2" onClick="window.location='/reading.php?lesson_id=747&page_id=1'"/>
		<?php echo '</p>';
