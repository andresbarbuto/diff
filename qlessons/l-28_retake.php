<?php
// most recent edit - 6-29-16; to include dual pricing modules.
define('QUADODO_IN_SYSTEM', true);
require_once('../includes/header.php'); ?>
<title>
    Baptism Quiz Retake Assigned
</title>
<?php include '../start.php'; ?>
<div class="static pt-1 pb-2">
    <a name="top"></a>
    <?php

    // coming from an include on the Baptism Quiz, this should be unnecesary
    //Check to see if user is logged in.
    if ($qls->user_info['username'] != '') {
    ?>
    <?php
    $id = $qls->user_info['id']; // set user_id for no accidents
    //echo $id . "id<br/>";

    $lesson_id = 28; // set lesson_id for no accidents
    //echo $lesson_id . " lesson<br/>";

    ?>

    <h1 class="static-title">&quot;The Meaning of the Sacrament of Baptism&quot; Quiz Retake Assigned</h1>
    <p>
        <?
        $retake_max = getQls()->SQL->num_rows(getQls()->SQL->query("SELECT * FROM quiz_retakes WHERE lesson_id = 28 AND user_id = '$id'   and  created_at > DATE_SUB(NOW(), INTERVAL 2 WEEK)"));
        //echo $retake_max. 'retake<br/>';

        if ($retake_max>=4) {
echo "<strong><p style='color: black'>Our records indicate that you have already been issued the maximum number of retakes for this lesson. In order to receive a passing score, please review your questions and answers. For each question that you missed, write up a short explanation describing the correct answer. Submit your answers in a document or email reply to <em>documents@catechismclass.com</em> for grading. 
<br/><br/>The answers to all questions may be combined in one email but follow this format for all questions to which you are submitting an answer:
   <li> The wording of the question</li>
   <li> Your submitted answer</li>
   <li> The correct answer</li>
   <li> Your written explanation to demonstrate why the correct answer is correct.</li>
Include your name, email address, and USER ID number - ". $id. " -  in your submission.</p></strong>"; ?>
        <br/><br/>
        <input type="button" value="Return to My Quizzes" class="btn btn-outline-dark mb-2" onClick="window.location='/quiz_results.php'"/>&nbsp;&nbsp;<br/><br/>
        <?php
        include '../info_columns.php';
        include '../bottom.php';
        exit;
    } else {
        $insert_date= date('Y-m-d G:i:s');
        $cupdate =  getQls()->SQL->query("INSERT INTO quiz_retakes VALUES ('', \"$id\",\"$lesson_id\",\"1\", '$insert_date')");
        // update data in mysql database
        $lupdate = getQls()->SQL->query("UPDATE quiz_results SET may_retake=1 WHERE user_id=$id AND lesson_id=28 ORDER BY id DESC LIMIT 1;") or die(getQls()->SQL->error());

        // if successfully updated.
        if ($lupdate) {
            // get Quiz info 
            $quiz_info = getQls()->SQL->query("SELECT id
                FROM quiz_results
                WHERE user_id = '$id' AND lesson_id = '28'
                ORDER BY lesson_id ASC, created_at DESC"
            );

            if ($row_quiz_info = getQls()->SQL->fetch_row($quiz_info)) {
                $rtupdate = getQls()->SQL->query("UPDATE reading_timed 
                    SET quiz_result_id='$row_quiz_info[0]'
                    WHERE user_id='$id' AND lesson_id='28' AND quiz_result_id='0' 
                    LIMIT 1") or die(getQls()->SQL->error());

                $now = gmdate("Y-m-d H:i:s");
                getQls()->SQL->query("INSERT IGNORE INTO reading_timed (user_id, lesson_id, created_at, reading_time) 
                    VALUES ('$id', '28', '$now', '00:00:00') ") or die(getQls()->SQL->error());
            }

            echo '<p>Retake Granted.</p>' ;
            echo '<p>To retake this Quiz &nbsp;&nbsp;';
        ?>
        <input type="button" value="Return to Lesson" class="btn btn-success mb-2" onClick="window.location='/reading.php?lesson_id=28&page_id=1'"/>
        <?php echo '</p>';
        //include '../email_retake_granted.php'; include when fixed
    } else {
        echo "ERROR";
    }
        }

        ?> </p>
    <!--<a href="lessons.php">Return to My Lessons</a>-->
    <hr />

    <?php }
// If not logged in, display links to log in and register
else {
    echo "<script type='text/javascript'> window.location= '../mustRegister.php' </script>";  }
include '../info_columns.php'; ?>
</div>
<?php include '../bottom.php'; ?>
