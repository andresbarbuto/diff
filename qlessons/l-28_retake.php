...
<div class="static pt-1 pb-2" id="top">
    ...
    if (!empty($_SESSION['username'])) {
        ...
        <?php
        $retake_max = mysqli()->query("SELECT * FROM quiz_retakes WHERE lesson_id = 28 AND user_id = '$id' AND created_at > DATE_SUB(NOW(), INTERVAL 2 WEEK)")->num_rows;
        ...
        if ($retake_max >= 6) {
            ...
            $lupdate = mysqli()->query("UPDATE quiz_results SET may_retake=1 WHERE user_id=$id AND lesson_id=28 ORDER BY id DESC LIMIT 1;") or die(mysqli()->error);

            // if successfully updated.
            if ($lupdate) {
                // get Quiz info 
                $quiz_info = mysqli()->query("SELECT id FROM quiz_results WHERE user_id = '$id' AND lesson_id = '28' ORDER BY lesson_id ASC, created_at DESC");

                if ($row_quiz_info = $quiz_info->fetch_row()) {
                    $rtupdate = mysqli()->query("UPDATE reading_timed SET quiz_result_id='$row_quiz_info[0]' WHERE user_id='$id' AND lesson_id='28' AND quiz_result_id='0' LIMIT 1") or die(mysqli()->error);

                    $now = gmdate("Y-m-d H:i:s");
                    mysqli()->query("INSERT IGNORE INTO reading_timed (user_id, lesson_id, created_at, reading_time) VALUES ('$id', '28', '$now', '00:00:00') ") or die(mysqli()->error);
                }
                ?>
