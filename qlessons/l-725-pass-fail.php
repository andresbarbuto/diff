...
$recent = $qls->SQL->query("SELECT * FROM purchases 
    ...
    LIMIT 1");
if ($l725PassFailPhp_hasRecent = $qls->SQL->fetch_array($recent)) {
...
$l725PassFailPhp_certReqQuery = $qls->SQL->query("SELECT cert_time, UNIX_TIMESTAMP(DATE_SUB(NOW(), INTERVAL 2 WEEK)) AS deadline
    ...
    LIMIT 1");
if ($l725PassFailPhp_certReqResult = $qls->SQL->fetch_assoc($l725PassFailPhp_certReqQuery)) {
...
if ($lesson_id == 725) {
    if ($avg <= 89) {
        ...
        if (isset($quizPhp_quizResult) && is_array($quizPhp_quizResult)) {
            ...
            // Or get from db
            $l725PassFailPhp_quizQuery = $qls->SQL->query("SELECT * 
                FROM quiz_results 
                WHERE lesson_id = '$lesson_id' AND user_id = '$id' 
                ORDER BY created_at DESC 
                LIMIT 1");
            $l725PassFailPhp_quizResult = $qls->SQL->fetch_assoc($l725PassFailPhp_quizQuery);
        ...
        if ($pDate != '' && strtotime($pDate) > strtotime($l725PassFailPhp_lastQuiz)) {
            ...
            <a href='reading.php?lesson_id=725&page_id=1' class='btn btn-success mt-2 mb-3'><big>Begin the Lesson</big></a></h5>";
        } else {
            ...
            if ($isFeeExists) {
                if ($tag == 1) {
                    echo "<h5>You have successfully completed the Godparent for a First Communion Preparation Program Quiz. 
                        ...
                        <a href='cert-request.php?nature=7' class='btn btn-success mt-2 mb-3' target='_blank'><big>CLICK HERE TO REQUEST</big> </a> </h5>";
                } else {
                    echo "<h5>You are requesting a certificate against a previously used course. Please purchase the lesson again in order to continue with your certificate request. <br/><a href='/shop/lessons_detail.php?id=725' class='btn btn-success mt-2 mb-3' > <big>Purchase New a Lesson</big> </a></h5><br/>";
                }
            } else {
                $l725PassFailPhp_processingFeeButton = '<p><big>
