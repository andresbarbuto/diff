...
if ($lesson_id == 747) {
    if ($avg <= 89) {
        ...
        $recent = $qls->SQL->query("SELECT * FROM purchases 
            ...
            LIMIT 1");
        if ($l747PassFailPhp_hasS59Recent = $qls->SQL->fetch_array($recent)) {
        ...
        if ($tag > 0) {
            ...
            $l747PassFailPhp_certReqQuery = $qls->SQL->query("SELECT cert_time, UNIX_TIMESTAMP(DATE_SUB(NOW(), INTERVAL 2 WEEK)) AS deadline
                ...
                LIMIT 1");
            if ($l747PassFailPhp_certReqResult = $qls->SQL->fetch_assoc($l747PassFailPhp_certReqQuery)) {
                ...
                if ($l747PassFailPhp_certTime < $l747PassFailPhp_deadline) $tag = 0;
            }
            ...
            if (isset($quizPhp_quizResult) && is_array($quizPhp_quizResult)) {
                ...
                // Or get from db
                $l747PassFailPhp_quizQuery = $qls->SQL->query("SELECT * 
                    FROM quiz_results 
                    WHERE lesson_id = '$lesson_id' AND user_id = '$id' 
                    ORDER BY created_at DESC 
                    LIMIT 1");
                $l747PassFailPhp_quizResult = $qls->SQL->fetch_assoc($l747PassFailPhp_quizQuery);
            ...
            if ($pDate != '' && strtotime($pDate) > strtotime($l747PassFailPhp_lastQuiz)) {
                ...
                $l747PassFailPhp_isFeeExists = ($row = $qls->SQL->fetch_assoc($result)) ? boolval($row["cnt"]) : false;
                if ($l747PassFailPhp_isFeeExists) {
                    ...
                    $l747PassFailPhp_processingFeeButton = '<p><big>
                ...
                if ($tag == 1) {
                    ...
                    $l747PassFailPhp_checkOtherQuizQuery = $qls->SQL->query("
                        ...
                        GROUP BY lesson_id");
                    while ($row = $qls->SQL->fetch_assoc($l747PassFailPhp_checkOtherQuizQuery)) {
                        switch ($row["lesson_id"]) {
                            case '745': $l747PassFailPhp_l745Score = round(($row["score"] * 100), 2); break;
                            case '746': $l747PassFailPhp_l746Score = round(($row["score"] * 100), 2); break;
                            case '748': $l747PassFailPhp_l748Score = round(($row["score"] * 100), 2); break;
                            case '749': $l747PassFailPhp_l749Score = round(($row["score"] * 100), 2); break;
                        }
                    }
