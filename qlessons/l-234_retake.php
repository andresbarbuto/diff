...
<title>Pre Cana Preparation Quiz Retake Assigned</title>
...
<div class="static pt-1 pb-2">  
    <?php if ($qls->user_info['username'] != '') { //Check to see if user is logged in.
        ...
        <p><?php
            $retake_max = getQls()->SQL->num_rows(getQls()->SQL->query("SELECT * FROM quiz_retakes WHERE lesson_id = 234 AND user_id = '$id' and created_at > DATE_SUB(NOW(), INTERVAL 2 WEEK)"));
