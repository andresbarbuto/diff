...
if ($lesson_id == 86) {
    if ($avg <= 89) {
        ...
        if ($l86PassFailPhp_hasP23Recent = $recent->fetch_array()) {
        ...
        if ($tag == 1 || $tag == -2) {
            ...
            if ($l86PassFailPhp_hasRecent = $recent->fetch_array()) {
                $tag = 1;
        ...
        if ($tag > 0) {
            ...
            $l86PassFailPhp_certReqQuery = mysqli()->query("SELECT cert_time, UNIX_TIMESTAMP(DATE_SUB(NOW(), INTERVAL 2 WEEK)) AS deadline
                FROM support_tickets
                WHERE user_id = '$id'
            ...
            if ($l86PassFailPhp_certReqResult = $l86PassFailPhp_certReqQuery->fetch_assoc()) {
                ...
                if ($l86PassFailPhp_certTime < $l86PassFailPhp_deadline)
                    $tag = 0;
            }
            ...
            if (isset($quizPhp_quizResult) && is_array($quizPhp_quizResult)) {
                ...
                // Or get from db
                $l86PassFailPhp_quizQuery = mysqli()->query("SELECT * 
                    FROM quiz_results 
                    WHERE lesson_id = '$lesson_id' AND user_id = '$id' 
                    ORDER BY created_at DESC 
                    LIMIT 1");
                $l86PassFailPhp_quizResult = $l86PassFailPhp_quizQuery->fetch_assoc();
            ...
            if ($pDate != '' && strtotime($pDate) > strtotime($l86PassFailPhp_lastQuiz)) {
                ...
                if (($row = $rs->fetch_assoc()) && $row["quiz_result_id"]) {
                    ...
                    mysqli()->query("INSERT IGNORE INTO reading_timed (user_id, lesson_id, created_at, reading_time) VALUES  ('$id', '86', '$now', '00:00:00'), ('$id', '87', '$now', '00:00:00') $l86PassFailPhp_insertReadingQuery");
                }
                ...
                <a href='reading.php?lesson_id=86&page_id=1' class='btn btn-success mt-2 mb-3 big'>Begin the Lesson</a></h5>";
            } else {
                ...
                $l86PassFailPhp_isFeeExists = ($row = $result->fetch_assoc()) ? boolval($row["cnt"]) : false;
                if ($l86PassFailPhp_isFeeExists) {
                    $l86PassFailPhp_certRequestButton = "<h5>You have successfully completed the $l86PassFailPhp_program Quiz. 
                        ...
                        <a href='cert-request.php?nature=$l86PassFailPhp_nature' class='btn btn-success mt-2 mb-3 big' target='_blank'>CLICK HERE TO REQUEST </a>
                    </h5>";
                } else {
                    $l86PassFailPhp_processingFeeButton = '<p class="big">
                ...
                if ($tag == 1) {
                    ...
                    $l86PassFailPhp_checkOtherQuizQuery = mysqli()->query("SELECT MAX(score) AS score, lesson_id
                        FROM quiz_results 
                        WHERE lesson_id = 87
                            AND user_id = '$id' 
                    ...
                    if ($row = $l86PassFailPhp_checkOtherQuizQuery->fetch_assoc()) {
                    ...
                    while ($row = $l86PassFailPhp_checkOtherQuizQuery->fetch_assoc()) {
                        if ($row["lesson_id"] == "87")
                            $l86PassFailPhp_l87Score = round(($row["score"] * 100), 2);
                        else if ($row["lesson_id"] == "234")
                            $l86PassFailPhp_l234Score = round(($row["score"] * 100), 2);
                    }
                    ...
                    echo "<h5>You are requesting a certificate against a previously used course. Please purchase the $l86PassFailPhp_program again in order to continue with your certificate request. <br><a href='$l86PassFailPhp_shopUrl' class='btn btn-success mt-2 mb-3 big' > Purchase </a></h5><br>";
                }
            }
        }
    }
