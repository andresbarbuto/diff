...
if ($l724PassFailPhp_hasRecent = $recent->fetch_array()) {
...
$l724PassFailPhp_certReqQuery = mysqli()->query("SELECT cert_time, UNIX_TIMESTAMP(DATE_SUB(NOW(), INTERVAL 2 WEEK)) AS deadline
...
if ($l724PassFailPhp_certReqResult = $l724PassFailPhp_certReqQuery->fetch_assoc()) {
    ...
    if ($l724PassFailPhp_certTime < $l724PassFailPhp_deadline)
        $tag = 0;
}
...
if ($lesson_id == 724) {
    if ($avg <= 89) {
        ...
        if (isset($quizPhp_quizResult) && is_array($quizPhp_quizResult)) {
            ...
            // Or get from db
            $l724PassFailPhp_quizQuery = mysqli()->query("SELECT * 
                FROM quiz_results 
                WHERE lesson_id = '$lesson_id_escaped' AND user_id = '$id' 
                ORDER BY created_at DESC 
                LIMIT 1");
            $l724PassFailPhp_quizResult = $l724PassFailPhp_quizQuery->fetch_assoc();
        ...
        if ($pDate != '' && strtotime($pDate) > strtotime($l724PassFailPhp_lastQuiz)) {
            ...
            <a href='reading.php?lesson_id=724&page_id=1' class='btn btn-success mt-2 mb-3 big'>Begin the Lesson</a></h5>";
        } else {
            ...
            if ($isFeeExists) {
                if ($tag == 1) {
                    echo "<h5>You have successfully completed the Sponsor for a Confirmation Preparation Program Quiz. 
                        ...
                        <a href='cert-request.php?nature=8' class='btn btn-success mt-2 mb-3 big' target='_blank'>CLICK HERE TO REQUEST </a> </h5>";
                } else {
                    echo "<h5>You are requesting a certificate against a previously used course. Please purchase the lesson again in order to continue with your certificate request. <br><a href='/shop/lessons_detail.php?id=724' class='btn btn-success mt-2 mb-3 big' > Purchase a New Lesson </a></h5><br>";
                }
            } else {
                $l724PassFailPhp_processingFeeButton = '<p class="big">
