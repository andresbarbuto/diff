...
$quiz_exists = mysqli()->query("SELECT * FROM quiz_results WHERE lesson_id = 85 AND user_id = '$id' ORDER BY created_at DESC LIMIT 1")->num_rows;

$may_retake_query = mysqli()->query("SELECT may_retake, doc_flag, created_at FROM quiz_results WHERE lesson_id = 85 AND user_id = '$id' ORDER BY created_at DESC LIMIT 1");
$may_retake = $may_retake_query->fetch_row();
$retake = $may_retake[0] ?? null;
$_85doc_flag = $may_retake[1] ?? null;
        ...
        if ($remedial_true == 1) {
            if ($lesson_id == 9) {
                if ($avg <= 79) {
                    ...
                    ?>
                    <input type="button" value="Retake this Quiz" class="btn btn-success  mb-2" onClick="window.location = 'qlessons/l-9_retake.php'">
                    <?php
            ...
            if ($lesson_id == 28) {
                if ($avg <= 79) {
                    ...
                    ?>
                    <input type="button" value="Retake this Quiz" class="btn btn-success  mb-2" onClick="window.location = 'qlessons/l-28_retake.php'">
                    <?php
                    echo "</p><hr style='height:1.5px; border:none; color:#000; background-color:#000;'>";
                } else {
                    $posible_tag28 = 1;
...
if ($remedial9 == 1) {
    ...
    $result3 = mysqli()->query($confirm_sp9);
    $has_sp9 = $result3->num_rows;
    ...
    $r1_quiz = mysqli()->query($remedial_quiz1)->num_rows;
    if ($r1_quiz >= 1) {
        $r1_detail = mysqli()->query($remedial_quiz1->fetch_array());
...
if ($remedial28 == 1) {
    ...
    $result4 = mysqli()->query($confirm_sp28);
    $has_sp28 = $result4->num_rows;
    ...
    $r2_quiz = mysqli()->query($remedial_quiz2)->num_rows;
    if ($r2_quiz >= 1) {
        $r2_detail = mysqli()->query($remedial_quiz2->fetch_array());
...
if (($remedial9 == 1) && ($remedial28 == 1)) {
    ...
    if (($l9_clear == 1) && ($l28_clear == 1)) {
        ...
        ?>
        <input type="button" value=" Click to Reset" class="btn btn-success mb-2" onClick="window.location = '/qlessons/baptism_retake.php'">
        <?php
        echo"</span></p>";
    } else {
        $tq1 = "SELECT * FROM purchased_lessons WHERE user_id= $id AND lesson_id= 9";
        $t1 = mysqli()->query($tq1)->num_rows;
        if ($t1 != 0) {
            ...
            if (($remedial9 == 1) && ($l9_clear != 1)) {
                ...
                ?>
                &nbsp;<input type="button" value="Read the Lesson" class="btn btn-success mb-2" onClick="window.location = '/reading.php?lesson_id=9&page_id=1'">
                <?php
                echo " </span></p>";
        ...
        if ($t2 != 0) {
            ...
            if (($remedial28 == 1) && ($l28_clear != 1)) {
                ...
                ?>
                &nbsp;<input type="button" value="Read the Lesson" class="btn btn-success mb-2" onClick="window.location = '/reading.php?lesson_id=28&page_id=1'">
                <?php
                echo " </span></p>";
