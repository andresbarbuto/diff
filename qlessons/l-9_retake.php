...
<div class="static pt-1 pb-2">
    ...
    <p>
        <?
        $retake_max = getQls()->SQL->num_rows(getQls()->SQL->query("SELECT * FROM quiz_retakes WHERE lesson_id = 9 AND user_id = '$id'   and  created_at > DATE_SUB(NOW(), INTERVAL 2 WEEK)"));
...
?>
        ...
        $lupdate = getQls()->SQL->query("UPDATE quiz_results SET may_retake=1 WHERE user_id=$id AND lesson_id=9 ORDER BY id DESC LIMIT 1;") or die(getQls()->SQL->error());
        ...
        if ($lupdate) {
            ...
            $quiz_info = getQls()->SQL->query("SELECT id
                FROM quiz_results
                WHERE user_id = '$id' AND lesson_id = '9'
            ...
            if ($row_quiz_info = getQls()->SQL->fetch_row($quiz_info)) {
                $rtupdate = getQls()->SQL->query("UPDATE reading_timed 
                    ...
                    WHERE user_id='$id' AND lesson_id='9' AND quiz_result_id='0' 
                ...
                getQls()->SQL->query("INSERT IGNORE INTO reading_timed (user_id, lesson_id, created_at, reading_time) 
                    VALUES ('$id', '9', '$now', '00:00:00') ") or die(getQls()->SQL->error());
            }
