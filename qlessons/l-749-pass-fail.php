...
if ($lesson_id == 749) {
    if ($avg <= 89) {
        ...
        $recent = $qls->SQL->query("SELECT * FROM purchases 
            ...
            LIMIT 1");
        if ($l749PassFailPhp_hasS59Recent = $qls->SQL->fetch_array($recent)) {
        ...
        if ($tag > 0) {
            ...
            $l749PassFailPhp_certReqQuery = $qls->SQL->query("SELECT cert_time, UNIX_TIMESTAMP(DATE_SUB(NOW(), INTERVAL 2 WEEK)) AS deadline
                ...
                LIMIT 1");
            if ($l749PassFailPhp_certReqResult = $qls->SQL->fetch_assoc($l749PassFailPhp_certReqQuery)) {
            ...
            if (isset($quizPhp_quizResult) && is_array($quizPhp_quizResult)) {
                ...
                // Or get from db
                $l749PassFailPhp_quizQuery = $qls->SQL->query("SELECT * 
                    FROM quiz_results 
                    WHERE lesson_id = '$lesson_id' AND user_id = '$id' 
                    ORDER BY created_at DESC 
                    LIMIT 1");
                $l749PassFailPhp_quizResult = $qls->SQL->fetch_assoc($l749PassFailPhp_quizQuery);
            ...
            if ($pDate != '' && strtotime($pDate) > strtotime($l749PassFailPhp_lastQuiz)) {
                ...
                $l749PassFailPhp_isFeeExists = ($row = $qls->SQL->fetch_assoc($result)) ? boolval($row["cnt"]) : false;
                if ($l749PassFailPhp_isFeeExists) {
                    ...
                    $l749PassFailPhp_processingFeeButton = '<p><big>
                ...
                if ($tag == 1) {
                    ...
                    $l749PassFailPhp_checkOtherQuizQuery = $qls->SQL->query("
                        ...
                        GROUP BY lesson_id");
                    while ($row = $qls->SQL->fetch_assoc($l749PassFailPhp_checkOtherQuizQuery)) {
