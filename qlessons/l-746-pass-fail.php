...
if ($lesson_id == 746) {
    if ($avg <= 89) {
        ...
        if ($l746PassFailPhp_hasS59Recent = $recent->fetch_array()) {
        ...
        if ($tag > 0) {
            ...
            $l746PassFailPhp_certReqQuery = mysqli()->query("SELECT cert_time, UNIX_TIMESTAMP(DATE_SUB(NOW(), INTERVAL 2 WEEK)) AS deadline
            ...
            if ($l746PassFailPhp_certReqResult = $l746PassFailPhp_certReqQuery->fetch_assoc()) {
                ...
                if ($l746PassFailPhp_certTime < $l746PassFailPhp_deadline)
                    $tag = 0;
            }
            ...
            if (isset($quizPhp_quizResult) && is_array($quizPhp_quizResult)) {
                ...
                // Or get from db
                $l746PassFailPhp_quizQuery = mysqli()->query("SELECT * 
                    FROM quiz_results 
                    WHERE lesson_id = '$lesson_id' AND user_id = '$id' 
                    ORDER BY created_at DESC 
                    LIMIT 1");
                $l746PassFailPhp_quizResult = $l746PassFailPhp_quizQuery->fetch_assoc();
            ...
            if ($pDate != '' && strtotime($pDate) > strtotime($l746PassFailPhp_lastQuiz)) {
                ...
                $l746PassFailPhp_isFeeExists = ($row = $result->fetch_assoc()) ? boolval($row["cnt"]) : false;
                if ($l746PassFailPhp_isFeeExists) {
                    ...
                    $l746PassFailPhp_processingFeeButton = '<p class="big">
                ...
                if ($tag == 1) {
                    ...
                    $l746PassFailPhp_checkOtherQuizQuery = mysqli()->query("SELECT MAX(score) AS score, lesson_id
                        FROM quiz_results 
                    ...
                    while ($row = $l746PassFailPhp_checkOtherQuizQuery->fetch_assoc()) {
                        switch ($row["lesson_id"]) {
                            case '745': $l746PassFailPhp_l745Score = round(($row["score"] * 100), 2);
                                break;
                            case '747': $l746PassFailPhp_l747Score = round(($row["score"] * 100), 2);
                                break;
                            case '748': $l746PassFailPhp_l748Score = round(($row["score"] * 100), 2);
                                break;
                            case '749': $l746PassFailPhp_l749Score = round(($row["score"] * 100), 2);
                                break;
                        }
                    }
