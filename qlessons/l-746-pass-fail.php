...
if ($lesson_id == 746) {
    if ($avg <= 89) {
        ...
        $recent = $qls->SQL->query("SELECT * FROM purchases 
            ...
            LIMIT 1");
        if ($l746PassFailPhp_hasS59Recent = $qls->SQL->fetch_array($recent)) {
            $pDate = $l746PassFailPhp_hasS59Recent["created_at"];
        ...
        if ($tag > 0) {
            ...
            $l746PassFailPhp_certReqQuery = $qls->SQL->query("SELECT cert_time, UNIX_TIMESTAMP(DATE_SUB(NOW(), INTERVAL 2 WEEK)) AS deadline
                ...
                LIMIT 1");
            if ($l746PassFailPhp_certReqResult = $qls->SQL->fetch_assoc($l746PassFailPhp_certReqQuery)) {
                $l746PassFailPhp_certTime = $l746PassFailPhp_certReqResult['cert_time'];
                $l746PassFailPhp_deadline = $l746PassFailPhp_certReqResult['deadline'];
                if ($l746PassFailPhp_certTime < $l746PassFailPhp_deadline) $tag = 0;
            }

            // Wahyudi Edit (2022-08-04) Check if there is new purchase Series 59 compare to last quiz
            // if this script included on quiz.php
            $l746PassFailPhp_lastQuiz = gmdate("Y-m-d");
            if (isset($quizPhp_quizResult) && is_array($quizPhp_quizResult)) {
                $l746PassFailPhp_lastQuiz = $quizPhp_quizResult["created_at"];
            } else { 
                // Or get from db
                $l746PassFailPhp_quizQuery = $qls->SQL->query("SELECT * 
                    FROM quiz_results 
                    WHERE lesson_id = '$lesson_id' AND user_id = '$id' 
                    ORDER BY created_at DESC 
                    LIMIT 1");
                $l746PassFailPhp_quizResult = $qls->SQL->fetch_assoc($l746PassFailPhp_quizQuery);
                $l746PassFailPhp_lastQuiz = $l746PassFailPhp_quizResult["created_at"];
            }

            // if there is new purchase compare to last quiz
            if ($pDate != '' && strtotime($pDate) > strtotime($l746PassFailPhp_lastQuiz)) {
                ...
                $l746PassFailPhp_certRequestButton = $l746PassFailPhp_processingFeeButton = "";
                $l746PassFailPhp_isFeeExists = ($row = $qls->SQL->fetch_assoc($result)) ? boolval($row["cnt"]) : false;
                if ($l746PassFailPhp_isFeeExists) {
                    $l746PassFailPhp_certRequestButton = "<h5>You have successfully completed the Quinceañera Preparation Program Quiz. 
                    ...
                    $l746PassFailPhp_processingFeeButton = '<p><big>
                ...
                if ($tag == 1) {
                    $l746PassFailPhp_l745Score = $l746PassFailPhp_l747Score = $l746PassFailPhp_l748Score = $l746PassFailPhp_l749Score = -1;
                    $l746PassFailPhp_checkOtherQuizQuery = $qls->SQL->query("
                        SELECT MAX(score) AS score, lesson_id
                        FROM quiz_results 
                        WHERE lesson_id IN (745, 747, 748, 749) 
                            AND user_id = '$id' 
                            AND created_at >= '$pDate'
                        GROUP BY lesson_id");
                    while ($row = $qls->SQL->fetch_assoc($l746PassFailPhp_checkOtherQuizQuery)) {
                        switch ($row["lesson_id"]) {
                            case '745': $l746PassFailPhp_l745Score = round(($row["score"] * 100), 2); break;
                            case '747': $l746PassFailPhp_l747Score = round(($row["score"] * 100), 2); break;
                            case '748': $l746PassFailPhp_l748Score = round(($row["score"] * 100), 2); break;
                            case '749': $l746PassFailPhp_l749Score = round(($row["score"] * 100), 2); break;
                        }
                    }

                    $l746PassFailPhp_isL745Passed = $l746PassFailPhp_l745Score >= 90;
                    $l746PassFailPhp_isL747Passed = $l746PassFailPhp_l747Score >= 90;
                    $l746PassFailPhp_isL748Passed = $l746PassFailPhp_l748Score >= 90;
                    $l746PassFailPhp_isL749Passed = $l746PassFailPhp_l749Score >= 90;
                    if ($l746PassFailPhp_isL745Passed && $l746PassFailPhp_isL747Passed && $l746PassFailPhp_isL748Passed && $l746PassFailPhp_isL749Passed) {
                        if ($l746PassFailPhp_isFeeExists) {
                            echo $l746PassFailPhp_certRequestButton;
                        } else {
                            // echo $l746PassFailPhp_processingFeeButton;
                        }
                    } else {
                        // Wahyudi Edit (2022-08-04) Show text For successfull l746 but not completed S59 
                        ...
                        if ($l746PassFailPhp_l745Score >= 0 && !$l746PassFailPhp_isL745Passed) {
                            echo "<p><span style='color: black'>".$l746PassFailPhp_l745Score . "% is not a passing score for Quinceañera Lesson 1: Living a Holy Life. You may click to reset here. ";
                        ...
                        if ($l746PassFailPhp_l747Score >= 0 && !$l746PassFailPhp_isL747Passed) {
                            echo "<p><span style='color: black'>".$l746PassFailPhp_l747Score . "% is not a passing score for Quinceañera Lesson 3: Overview of the Catholic Faith Part 2. You may click to reset here. ";
                        ...
                        if ($l746PassFailPhp_l748Score >= 0 && !$l746PassFailPhp_isL748Passed) {
                            echo "<p><span style='color: black'>".$l746PassFailPhp_l748Score . "% is not a passing score for Quinceañera Lesson 4: The Sacramental Life. You may click to reset here. ";
                        ...
                        if ($l746PassFailPhp_l749Score >= 0 && !$l746PassFailPhp_isL749Passed) {
                            echo "<p><span style='color: black'>".$l746PassFailPhp_l749Score . "% is not a passing score for Quinceañera Lesson 5: Preparing for Quince Años. You may click to reset here. ";
