...
if ($lesson_id == 745) {
    if ($avg <= 89) {
        ...
        if ($l745PassFailPhp_hasS59Recent = $recent->fetch_array()) {
        ...
        if ($tag > 0) {
            ...
            $l745PassFailPhp_certReqQuery = mysqli()->query("SELECT cert_time, UNIX_TIMESTAMP(DATE_SUB(NOW(), INTERVAL 2 WEEK)) AS deadline
            ...
            if ($l745PassFailPhp_certReqResult = $l745PassFailPhp_certReqQuery->fetch_assoc()) {
            ...
            if (isset($quizPhp_quizResult) && is_array($quizPhp_quizResult)) {
                ...
                // Or get from db
                $l745PassFailPhp_quizQuery = mysqli()->query("SELECT * 
                    FROM quiz_results 
                    WHERE lesson_id = '$lesson_id' AND user_id = '$id' 
                    ORDER BY created_at DESC 
                    LIMIT 1");

                $l745PassFailPhp_quizResult = $l745PassFailPhp_quizQuery->fetch_assoc();
            ...
            if ($pDate != '' && strtotime($pDate) > strtotime($l745PassFailPhp_lastQuiz)) {
                ...
                $l745PassFailPhp_isFeeExists = ($row = $result->fetch_assoc()) ? boolval($row["cnt"]) : false;
                if ($l745PassFailPhp_isFeeExists) {
                    ...
                    $l745PassFailPhp_processingFeeButton = '<p class="big">
                ...
                if ($tag == 1) {
                    ...
                    $l745PassFailPhp_checkOtherQuizQuery = mysqli()->query("SELECT MAX(score) AS score, lesson_id
                        FROM quiz_results 
                    ...
                    while ($row = $l745PassFailPhp_checkOtherQuizQuery->fetch_assoc()) {
