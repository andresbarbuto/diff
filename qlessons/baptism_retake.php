...
<div class="static pt-1 pb-2" id="top">
    ...
    if (!empty($_SESSION['username'])) {
        ...
        <?php
        $retake_max = mysqli()->query("SELECT * FROM quiz_retakes WHERE lesson_id = 85 AND user_id = '$id' AND created_at > DATE_SUB(NOW(), INTERVAL 2 WEEK)")->num_rows;
        ...
        if ($retake_max >= 6) {
            ...
            $lupdate = mysqli()->query("UPDATE quiz_results SET may_retake=1 WHERE user_id=$id AND lesson_id=85 ORDER BY id DESC LIMIT 1;") or die(mysqli()->error);
