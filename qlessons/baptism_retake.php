...
<div class="static pt-1 pb-2">
    ...
    $lesson_id = 85; // set lesson_id for no accidents
    //echo $lesson_id . " lesson<br/>";

    ?>

    <h1 class="static-title">Baptism Quiz Retake Assigned</h1>
    <p>
        <?
        $retake_max = getQls()->SQL->num_rows(getQls()->SQL->query("SELECT * FROM quiz_retakes WHERE lesson_id = 85 AND user_id = '$id'   and  created_at > DATE_SUB(NOW(), INTERVAL 2 WEEK)"));
...
Include your name, email address, and USER ID number - ". $id. " -  in your submission.</p></strong>"; 
  ?>
        ...
        $lupdate = getQls()->SQL->query("UPDATE quiz_results SET may_retake=1 WHERE user_id=$id AND lesson_id=85 ORDER BY id DESC LIMIT 1;") or die(getQls()->SQL->error());
        ...
        if ($lupdate) {
            ...
            $quiz_info = getQls()->SQL->query("SELECT id
                FROM quiz_results
                WHERE user_id = '$id' AND lesson_id = '$lesson_id'
            ...
            if ($row_quiz_info = getQls()->SQL->fetch_row($quiz_info)) {
                $rtupdate = getQls()->SQL->query("UPDATE reading_timed 
                    ...
                    WHERE user_id='$id' AND lesson_id='$lesson_id' AND quiz_result_id='0' 
        ...
        ?>
        <input type="button" value="Return to Lesson" class="btn btn-success mb-2" onClick="window.location='/reading.php?lesson_id=85&page_id=1'"/>
        <?php echo '</p>';
